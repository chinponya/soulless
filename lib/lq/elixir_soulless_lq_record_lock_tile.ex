...
defmodule(Soulless.Lq.RecordLockTile) do
  @moduledoc false
  (
    defstruct(
      seat: 0,
    ...
    (
      (
        ...
        (
          ...
          def(decode!(bytes)) do
            parse_key_value(bytes, struct(Soulless.Lq.RecordLockTile))
          end
        )
      )
          ...
          {field, rest} =
            case(Protox.Decode.parse_key(bytes)) do
              ...
              {5, _, bytes} ->
                ...
                {[
                   operation:
                     msg.operation ++ [Soulless.Lq.OptionalOperationList.decode!(delimited)]
                 ], rest}
              ...
              {6, 2, bytes} ->
                ...
                {[zhentings: msg.zhentings ++ Protox.Decode.parse_repeated_bool([], delimited)],
                 rest}

              {6, _, bytes} ->
                {value, rest} = Protox.Decode.parse_bool(bytes)
                {[zhentings: msg.zhentings ++ [value]], rest}
    ...
    def(defs()) do
      %{
        ...
        5 => {:operation, :unpacked, {:message, Soulless.Lq.OptionalOperationList}},
        6 => {:zhentings, :packed, :bool},
