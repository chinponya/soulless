...
defmodule Soulless.Game.Lq.ReqResponseCaptcha do
  @moduledoc false
  defstruct check_id: 0, check_time: 0, result: "", client_version_string: "", type: 0, __uf__: []
      ...
      defp parse_key_value(bytes, msg) do
        {field, rest} =
          case Protox.Decode.parse_key(bytes) do
            ...
            {2, _, bytes} ->
              {value, rest} = Protox.Decode.parse_uint32(bytes)
              {[check_time: value], rest}

            {3, _, bytes} ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[result: Protox.Decode.validate_string(delimited)], rest}
  ...
  (
    ...
    def json_decode!(input, opts \\ []) do
      ...
      Protox.JsonDecode.decode!(
        input,
        Soulless.Game.Lq.ReqResponseCaptcha,
