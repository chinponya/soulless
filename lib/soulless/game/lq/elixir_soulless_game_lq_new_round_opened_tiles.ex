...
defmodule Soulless.Game.Lq.NewRoundOpenedTiles do
  @moduledoc false
  defstruct seat: 0, tiles: [], count: [], __uf__: []
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(Soulless.Game.Lq.NewRoundOpenedTiles))
        end
      )
    )
        ...
        {field, rest} =
          case Protox.Decode.parse_key(bytes) do
            ...
            {3, 2, bytes} ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[count: msg.count ++ Protox.Decode.parse_repeated_uint32([], delimited)], rest}

            {3, _, bytes} ->
              {value, rest} = Protox.Decode.parse_uint32(bytes)
              {[count: msg.count ++ [value]], rest}
  ...
  (
    ...
    def json_decode!(input, opts \\ []) do
      ...
      Protox.JsonDecode.decode!(
        input,
        Soulless.Game.Lq.NewRoundOpenedTiles,
  ...
  (
    ...
    def defs() do
      %{
        ...
        2 => {:tiles, :unpacked, :string},
        3 => {:count, :packed, :uint32}
      }
    end
    ...
    def defs_by_name() do
      %{
        count: {3, :packed, :uint32},
        seat: {1, {:scalar, 0}, :uint32},
        tiles: {2, :unpacked, :string}
      }
    end
  )
