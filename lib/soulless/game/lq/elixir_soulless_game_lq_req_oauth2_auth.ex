...
defmodule Soulless.Game.Lq.ReqOauth2Auth do
  @moduledoc false
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(Soulless.Game.Lq.ReqOauth2Auth))
        end
      )
    )
        ...
        {field, rest} =
          case Protox.Decode.parse_key(bytes) do
            ...
            {3, _, bytes} ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[uid: Protox.Decode.validate_string(delimited)], rest}
