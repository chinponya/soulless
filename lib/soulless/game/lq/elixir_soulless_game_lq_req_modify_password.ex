...
defmodule Soulless.Game.Lq.ReqModifyPassword do
  @moduledoc false
    ...
    [
      defp encode_new_password(acc, msg) do
        try do
          ...
          ArgumentError ->
            reraise Protox.EncodingError.new(:new_password, "invalid field value"), __STACKTRACE__
        end
      end,
      defp encode_old_password(acc, msg) do
        try do
          if msg.old_password == "" do
            ...
            [acc, "\x12", Protox.Encode.encode_string(msg.old_password)]
          end
        rescue
          ArgumentError ->
            reraise Protox.EncodingError.new(:old_password, "invalid field value"), __STACKTRACE__
        end
      end,
      defp encode_secure_token(acc, msg) do
        try do
          if msg.secure_token == "" do
            ...
            [acc, "\x1A", Protox.Encode.encode_string(msg.secure_token)]
          end
        rescue
          ArgumentError ->
            reraise Protox.EncodingError.new(:secure_token, "invalid field value"), __STACKTRACE__
        end
      end
    ]
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(Soulless.Game.Lq.ReqModifyPassword))
        end
      )
    )
        ...
        {field, rest} =
          case Protox.Decode.parse_key(bytes) do
            ...
            {1, _, bytes} ->
              ...
              {[new_password: Protox.Decode.validate_string(delimited)], rest}

            {2, _, bytes} ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[old_password: Protox.Decode.validate_string(delimited)], rest}

            {3, _, bytes} ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[secure_token: Protox.Decode.validate_string(delimited)], rest}
  ...
  (
    ...
    def json_decode!(input, opts \\ []) do
      ...
      Protox.JsonDecode.decode!(
        input,
        Soulless.Game.Lq.ReqModifyPassword,
